    ...
    for model, search_space in zip(models, search_spaces):
        ...
        for feature_set in feature_sets:
            RS = RandomizedSearchCV(estimator=model,
                            ...
                            cv=5,
                            verbose=4)
            RS.fit(X_train[feature_set], y_train)
            best_model = RS.best_estimator_
            y_pred = best_model.predict(X_test[feature_set])
            ...
            best_scores = {}
            for metric in RS.scoring:
                key = f'mean_test_{metric}'
                best_scores[metric] = RS.cv_results_[key][best_idx]
        ...
        with pd.ExcelWriter('model_report_diabetes_randomized_search.xlsx', mode='a', if_sheet_exists='replace') as writer:
